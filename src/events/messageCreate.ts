...
import { checkPermissions, sendTimedMessage } from '../services';
...
const event: BotEvent = {
  ...
  execute: async (message: Message) => {
    ...
    if (!command) {
      const commandFromAlias = message.client.commands.find((command: Command) => command.aliases.includes(args[0]));
      if (commandFromAlias) {
